...
import { Search, Filter, SortAsc, SortDesc, Building, Star, Phone, Mail } from 'lucide-react';
...
const mockRealtors: Realtor[] = [
  {
    ...
    rating: 4.9,
    status: 'active',
    description: 'Специалист по элитной недвижимости. Помогу найти ваш идеальный дом.',
...
const RealtorsPage: React.FC = () => {
  ...
  const paginatedRealtors = filteredAndSortedRealtors.slice(startIndex, startIndex + itemsPerPage);
    ...
    <div className="py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-sm font-medium text-gray-500">Всего риэлторов</div>
          <div className="mt-1 text-2xl font-semibold text-gray-900">{mockRealtors.length}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-sm font-medium text-gray-500">Средний рейтинг</div>
          <div className="mt-1 text-2xl font-semibold text-blue-600">
            {(mockRealtors.reduce((sum, r) => sum + r.rating, 0) / mockRealtors.length).toFixed(1)}
          </div>
        </div>
      </div>
        ...
        <ul className="divide-y divide-gray-200">
          {paginatedRealtors.length > 0 ? (
                ...
                <div className="px-4 py-4 sm:px-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="flex items-center">
                          <h3 className="text-lg font-medium text-gray-900">{realtor.name}</h3>
                          <span className={`ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusClass(realtor.status)}`}>
                            {getStatusText(realtor.status)}
                          </span>
                        </div>
                        <div className="flex items-center mt-1">
                          <Star className="h-4 w-4 text-yellow-400 fill-current" />
                          <span className="ml-1 text-sm text-gray-600">{realtor.rating}</span>
                          <span className="mx-2 text-gray-300">•</span>
                          <span className="text-sm text-gray-600">{realtor.experience} лет опыта</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      <div className="text-sm text-gray-900">
                        <span className="font-medium">Продаж:</span> {realtor.propertiesSold}
                      </div>
                      <div className="mt-1 text-sm text-gray-900">
                        <span className="font-medium">Регистрация:</span> {new Date(realtor.joinedDate).toLocaleDateString('ru-RU')}
                      </div>
                    </div>
                  ...
                  <div className="mt-3">
                    <p className="text-sm text-gray-600">{realtor.description}</p>
                  </div>
                </div>
              </li>
            ))
            ...
            <li className="px-4 py-8 text-center">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">Риэлторы не найдены</h3>
              <p className="mt-1 text-sm text-gray-500">
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="text-sm text-gray-700">
            ...
            <span className="font-medium">
              {Math.min(startIndex + itemsPerPage, filteredAndSortedRealtors.length)}
            </span>{' '}
            из <span className="font-medium">{filteredAndSortedRealtors.length}</span> риэлторов
          </div>
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`relative inline-flex items-center px-3 py-2 border text-sm font-medium rounded-md ${currentPage === pageNum
